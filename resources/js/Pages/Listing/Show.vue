...
<template>
    ...
    class="bg-slate-800 text-white mb-6 p-6 rounded-md font-medium flex
items-center justify-between">
        <p>This listing is {{listing.approved ? "Approved" : "Disapproved"}}</p>
        <button class="bg-slate-600 px-3 py-1 rounded-md cursor-pointer">
                ...
                <div class="flex items-end justify-between mb-2">
                    ...
                    <div v-if="canModify"
                        ...
                        <Link :href="route('listing.edit', listing.id)"
                            class="bg-green-500  rounded-md text-white px-6
                        py-2 hover:outline outline-green-500 outline-offset-2">
                            Edit
                        </Link>

                        <button @click="deleteListing"
                                type="button"
                                class="bg-red-500  rounded-md text-white px-6
                        py-2 hover:outline outline-red-500 outline-offset-2 cursor-pointer"
                        >
            ...
            <div v-if="listing.tags" class="mb-6">
                ...
                <div class="flex items-center gap-3 mt-3">
                    <div v-for="tag in listing.tags.split(',')" :key="tag">
                        <Link
                            :href="route('home', { tag })"
                            class="bg-slate-500 text-white  px-3 py-1.5 rounded-full hover:bg-slate-700 dark:hover:bg-slate-900"
                        >
                            {{ tag }}
                        </Link>
                    </div>
